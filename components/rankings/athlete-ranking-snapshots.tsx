    ...
    <section key={snapshot.canonicalId} className="border border-white/15 bg-surface">
      <header className="grid gap-5 border-b border-white/10 p-5 sm:p-7 lg:grid-cols-[1fr_auto] lg:items-start">
        <div>
          ...
          <p className="mt-3 text-sm leading-6 text-muted">{[snapshot.discipline, snapshot.liftFormat, snapshot.division, snapshot.weightClass, snapshot.equipment, snapshot.geographicScope].filter(Boolean).join(" · ")}</p>
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[38rem] border-collapse text-left">
          ...
          <thead className="font-mono text-[0.68rem] font-bold uppercase tracking-[0.11em] text-muted"><tr><th scope="col" className="px-5 py-4">Rank</th><th scope="col" className="px-5 py-4">Athlete</th><th scope="col" className="px-5 py-4">Source total / score</th><th scope="col" className="px-5 py-4">Rank change</th></tr></thead>
          <tbody className="divide-y divide-white/10">{snapshot.entries.map((entry) => <tr key={entry.canonicalId}><td className="px-5 py-4 font-display text-2xl font-black text-ink">{entry.position ?? "—"}</td><th scope="row" className="px-5 py-4 text-sm font-bold uppercase text-ink">{entry.athleteSlug ? <Link href={`/athletes/${entry.athleteSlug}`} className="hover:text-accent">{entry.athleteName}</Link> : <>{entry.athleteName}<span className="mt-1 block font-mono text-[0.65rem] text-muted">External entry — identity review pending</span></>}</th><td className="px-5 py-4 font-mono text-xs text-muted">{entry.sourceValue ?? (entry.points !== undefined ? `${entry.points} pts` : entry.rating !== undefined ? entry.rating : "Not provided")}</td><td className="px-5 py-4 font-mono text-xs text-muted">{entry.previousPosition && entry.position ? `${entry.previousPosition - entry.position > 0 ? "+" : ""}${entry.previousPosition - entry.position}` : "New / unavailable"}</td></tr>)}</tbody>
        </table>
      </div>
